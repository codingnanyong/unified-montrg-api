"""Service layer for CTM data operations."""
...
async def fetch_ctm_data_by_pid(
    pid: int,
    limit: int,
    db_alias: Optional[str] = None,
) -> Optional[HmiDataResponse]:
    """Fetch CTM data by PID without additional filters."""
    return await _fetch_ctm_data(pid=pid, limit=limit, db_alias=db_alias)


async def fetch_ctm_data_by_pid_and_range(
    pid: int,
    limit: int,
    ...
    """Fetch CTM data filtered by an explicit date-key range."""
    ...
    return await _fetch_ctm_data(pid=pid, limit=limit, db_alias=db_alias, start=start_dt, end=end_dt)


async def fetch_ctm_data_by_pid_and_date_key(
    pid: int,
    limit: int,
    date_key: str,
    db_alias: Optional[str] = None,
) -> Optional[HmiDataResponse]:
    """Fetch CTM data filtered by a date key (YYYY, YYYYMM, YYYYMMDD)."""
    start, end = _resolve_date_key(date_key)
    return await _fetch_ctm_data(pid=pid, limit=limit, db_alias=db_alias, start=start, end=end)


async def fetch_ctm_data_latest(
    pid: int,
    db_alias: Optional[str] = None,
) -> Optional[HmiDataResponse]:
    """Fetch the most recent CTM data for a given PID."""
...
async def fetch_ctm_data_latest_by_pids(
    pids: List[int],
    db_alias: Optional[str] = None,
) -> List[HmiDataResponse]:
    """Fetch the most recent CTM data for multiple PIDs (only today's data).
